...
import { FiStar, FiCalendar, FiMapPin, FiDollarSign, FiUsers } from 'react-icons/fi';
...
const OrganizerDetails = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (organizerData) {
          ...
          setPackages(Array.isArray(organizerPackages) ? organizerPackages : []);
        } else {
  ...
  const handleRequestEvent = (packageId) => {
    ...
    navigate(`/requester/requests/new?packageId=${packageId}&organizerId=${id}`);
  };
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center">
          ...
          <p className="text-lg text-gray-600 mb-8">{error || "The organizer you're looking for doesn't exist or has been removed."}</p>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden mb-10">
        <div className="p-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">{organizer.organizationName || 'Organization'}</h1>
              <p className="text-gray-600 mb-4">Managed by {organizer.username || 'User'}</p>
            ...
            <div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center text-gray-600">
                  ...
                  {organizer.mobileNumber || 'No phone number provided'} </div>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Event Packages */}
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Available Event Packages</h2>
      
      {!Array.isArray(packages) || packages.length === 0 ? (
        <div className="bg-white rounded-lg shadow-md p-8 text-center">
          <p className="text-gray-600">This organizer doesn't have any event packages yet.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {packages.map((pkg) => (
            <div key={pkg.id} className="bg-white rounded-lg shadow-md overflow-hidden">
              ...
              <div className="p-6">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">{pkg.title || 'Unnamed Package'}</h3>
                ...
                <p className="text-gray-600 mb-6">{pkg.description || 'No description available'}</p>
